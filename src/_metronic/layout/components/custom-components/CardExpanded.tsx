    ...
    useEffect(() => {
        if (quillRef.current) {
            const quillInstance = new Quill(quillRef.current, {
                readOnly: true,
                theme: "bubble", // No UI, just content
                modules: { toolbar: false }, // Ensure no toolbar
            });

            quillInstance.root.innerHTML = content; // Load HTML content
        }
    }, [content]);

    return <div ref={quillRef} className="quill-content" />;
};
interface CardProps {
    id: string;
...
const TicketCardExpanded: React.FC<CardProps> = ({
                                             id,
                                             status,
                                             title,
                                             description,
                        ...
                        <div className="col-md-9">
                            <div className="d-flex flex-column title-desc">
                                ...
                                <p
                                    className={` mb-0 `}
                                >
                                    <HeadlessQuillViewer content={description} />;
                                </p>
                            </div>
                        </div>
                    </div>
...
export default TicketCardExpanded;
