...
const TicketsPage: React.FC = () => {
  ...
  const [ticketIdsWithReplyUnread, setTicketIdsWithReplyUnread] = useState<
    any[]
  >([]);
  ...
  const [totalTicketsFetched, setTotalTicketsFetchedAtom] = useAtom(
    totalTicketsFetchedAtom
  ); // Set the total tickets per query of fetched in this instance only
    ...
    try {
      const updatedTickets = intervalFetchedTickets.filter(
        (intervalTicket: any) => {
          ...
          return (
            initialTicket &&
            intervalTicket.last_reply_date != null &&
  ...
  >({
        ...
        if (
          ...
          filterNewTickets().then((newTickets) => {
            if (newTickets.length > 0) {
              setTicketIdsWithReplyUnread((prevIds) => [
                ...prevIds,
                ...newTickets,
              ]);
          ...
          setInitialFetchedTicketsTotal(intervalFetchedTicketsRes.totalcount);
        }
      }
    ...
    onError: (error) => {
      console.log(
        "There was an error in searching for tickets: " + error.message
      );
    },
    onSettled: () => {
  ...
  useEffect(() => {
    ...
    const hasValues = (obj: object): boolean =>
  ...
  const fetchTickets = async () => {
    try {
      ...
      if (backendFilter.to)
        filterBody.opening_date.to_date = backendFilter.to.value;
      if (backendFilter.isStarred)
        filterBody.starred = backendFilter.isStarred.value === "true" ? 1 : 0;
  ...
  useEffect(() => {
    ...
    const filtered = sortedTickets.filter((ticket) => {
      ...
      const matchesAssignee =
        ...
        assignees.some(
          (assignee) => String(assignee.id) === frontFilter.assignee.value
        );
  ...
  useEffect(() => {
    if (toolbarNewTickets) {
      if (intervalFetchedTicketsResponse) {
        ...
        setCurrentTotalTicketsAccumulation(
          intervalFetchedTicketsResponse.data.length
        );
  ...
  useEffect(() => {
    if (ticketChangeAssigneenOn) {
      console.log(
        "Ticket performing action on:",
        ticketChangeAssigneenOn.ticketId
      );
      const updatedTicket = tickets.find(
      ...
      if (updatedTicket) {
        ...
        if (!hasIds) {
          status = statusOptions.find(
            (option: { label: string; value: string }) => option.label === "new"
          ) || { value: "", label: "" };
        } else {
          status = statusOptions.find(
            ...
            <div className="d-flex flex-column justify-content-between">
              {isDataLoading ? (
                ...
                paginatedTickets.map((ticket) => {
                  ...
                  const assignees: Assignee[] = isEmptyAssignees
                    ? []
                    : parsedAssignees;
                  return (
                    <TicketCard
                      ...
                      reply_unread={
                        ticketIdsWithReplyUnread?.some(
                          (item) =>
                            item.id === ticket.id && item.is_read === "unread"
                        ) || false
                      }
                      date={ticket.date}
                      title={ticket.name}
